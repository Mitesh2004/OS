...
void cscan(int request[], int n, int head, int direction) {
    ...
    int total_seek_time = 0;
    int i, start;
    int boundary = 199; // Maximum disk size
    ...
    if (direction == 1) {  // Moving right (increasing order)
        for (i = start; i < n; i++) {
            total_seek_time += abs(request[i] - head);
            head = request[i];
            printf(" -> %d", head);
        ...
        // Jump to boundary, then to 0
        total_seek_time += abs(boundary - head);
        printf(" -> %d", boundary);
        head = 0;
        printf(" -> %d", head);

        // Continue from the lowest request
        for (i = 0; i < start; i++) {
            total_seek_time += abs(request[i] - head);
            head = request[i];
            printf(" -> %d", head);
    ...
    } else {  // Moving left (decreasing order)
        for (i = start - 1; i >= 0; i--) {
            total_seek_time += abs(request[i] - head);
            head = request[i];
            printf(" -> %d", head);
        ...
        // Jump to 0, then to boundary
        total_seek_time += abs(head - 0);
        printf(" -> %d", 0);
        head = boundary;
        total_seek_time += boundary; // Moving from 0 to 199
        printf(" -> %d", boundary);

        // Continue from the highest request
        for (i = n - 1; i >= start; i--) {
            total_seek_time += abs(request[i] - head);
            head = request[i];
            printf(" -> %d", head);
...
void cscan(int request[], int n, int head, int direction) {
    ...
    if (direction == 1) {  // Moving right (increasing order)
        ...
        printf(" -> %d", boundary);
        head = 0;
        total_seek_time += boundary; // Moving from 199 to 0
        printf(" -> %d", head);
